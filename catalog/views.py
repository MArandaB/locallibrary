...
def index(request):
    ...
    ultimoslibros = Book.objects.all().order_by('-id')[:10]
...
class BookListView(ListView):
    '''Vista genérica para lista de libros'''
    model = Book
    paginate_by = 15
...
class BookDetailView(DetailView):
    '''Vista genérica para detalle de libro'''
    model = Book


class AuthorListView(ListView):
    '''Vista genérica para lista de autores'''
    model = Author
    paginate_by = 15

class AuthorDetailView(DetailView):
    '''Vista genérica para detalle de autor'''
    model = Author
...
class SearchResultsListView(ListView):
    ...
    def get_queryset(self): # new
        query = self.request.GET.get('q','')#voy a guardar query para el contexto
        if query: 
            self.query = query
...
class LibrosPrestados(ListView):
    '''Vista genérica para lista de libros prestados'''
    model = Bookinstance
...
##Gestion de autores con vistas genéricas para
class AuthorCreate(CreateView):
    model = Author
    fields = ['first_name', 'last_name', 'date_of_birth', 'date_of_death']
    ##si funciona bien: success url
